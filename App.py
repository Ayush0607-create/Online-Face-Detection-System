...
def face_detect(image,sf,mn):
    ...
    face_obj = faceCascade.detectMultiScale(gray,sf,mn)
    for (x, y, w, h) in face_obj:
        i = i+1
...
def run():
    ...
    choice = st.sidebar.selectbox("Choose input source:", activities)
    link = '[Source Code](https://github.com/Ayush0607-create/Online-Face-Detection-System)'
    st.sidebar.markdown(link, unsafe_allow_html=True)
    if choice == 'Image':
        st.markdown(
            '''<h4 style='color: #c62a4b;'>*HaarCascade detector"</h4>''',
        ...
        if img_file is not None:
            ...
            st.markdown(
                '''<h4 style='color: #c62a4b;'>* Modify parameters to get better accuracy.</h4>''',
                unsafe_allow_html=True)
            ...
            if count == 0:
                ...
                st.success("Total faces detected : " + str(count))
    ...
    if choice == 'Webcam':
        st.markdown(
            '''<h4 style='color: #c62a4b;'>* Android device not supported."</h4>''',
            unsafe_allow_html=True)
